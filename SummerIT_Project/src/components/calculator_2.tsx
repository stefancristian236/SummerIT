...
const Calculator2 = () => {
  ...
  const calculate = (): string => {
    ...
    switch(operation) {
      ...
      case '-': result = previous - current; break;
      case '*': result = previous * current; break;
      case '/': error = current === 0 ? 'Error: Div by Zero' : null; result = current !== 0 ? previous / current : null; break;
      case '^': result = Math.pow(previous, current); break;
      case 'sqrtn': 
        error = current === 0 ? 'Error: Root by Zero' : (previous < 0 && current % 2 === 0 ? 'Error: Even root of negative' : null);
        if (!error) result = Math.pow(previous, 1/current);
        break;
      case 'mod': result = previous % current; break;
      case 'sqrt': error = current < 0 ? 'Error: Imaginary result' : null; if (!error) result = Math.sqrt(current); break;
      ...
      case 'sin_x': result = Math.sin(current); break;
      case 'cos_x': result = Math.cos(current); break;
      case 'tan_x': result = Math.tan(current); break;
      case 'sinh_x': result = Math.sinh(current); break;
      case 'cosh_x': result = Math.cosh(current); break;
      case 'tanh_x': result = Math.tanh(current); break;
      case 'asin_x': error = current < -1 || current > 1 ? 'Error: Domain (-1, 1)' : null; if (!error) result = Math.asin(current); break;
      case 'acos_x': error = current < -1 || current > 1 ? 'Error: Domain (-1, 1)' : null; if (!error) result = Math.acos(current); break;
      case 'atan_x': result = Math.atan(current); break;
      case 'n!': 
        error = (Math.floor(current) !== current || current < 0 || current > 170) ? 'Error: Integer 0-170 required' : null;
        if (!error) { result = 1; for(let i=1;i<=current;i++){result*=i;} }
        break;
      case 'ln': case 'log10': error = current <= 0 ? 'Error: Undefined for x <= 0' : null; if(!error) result = operation==='ln'?Math.log(current):Math.log10(current); break;
      case 'abs': result = Math.abs(current); break;
      case 'exp': result = Math.pow(Math.E, current); break;
      default: return displayValue;
    }

    if (error) return error;
    if (result !== null) return Number.parseFloat(result.toPrecision(10)).toString();
    return displayValue;
  };

  const equals = (): void => {
    const value = calculate();
    if (value.startsWith('Error')) { handleError(value); return; }
    setDisplayValue(value); setPrevValue(''); setOperation(''); setOverwrite(true);
  };

  const selectOperation = (nextOperation: string): void => {
    if (nextOperation === 'clear') { clearDisplay(); return; }
    if (nextOperation === 'del') { deleteLast(); return; }
    ...
    if(singleOperandOps.includes(nextOperation)){
      setPrevValue(displayValue); setOperation(nextOperation);
      const value = calculate();
      if(value.startsWith('Error')){ handleError(value); return; }
      setDisplayValue(value); setPrevValue(''); setOperation(''); setOverwrite(true); return;
    }

    if(prevValue && operation && !overwrite){ 
      const value = calculate();
      if(value.startsWith('Error')){ handleError(value); return; }
      setPrevValue(value); setDisplayValue(value);
    } else { setPrevValue(displayValue); }

    setOperation(nextOperation); setOverwrite(true);
  };

  const clearDisplay = (): void => { setDisplayValue('0'); setOperation(''); setPrevValue(''); setOverwrite(true); setMessage(''); };
  const deleteLast = (): void => { setDisplayValue(overwrite||displayValue.length<=1?'0':displayValue.slice(0,-1)); setOverwrite(true); };

  const setDigit = (digit: string): void => {
    if(message) return;
    if(digit === '.' && displayValue.includes('.')) return;
    if(digit==='e'){ setDisplayValue(Math.E.toFixed(10)); setOverwrite(true); return; }
    if(digit==='pi'){ setDisplayValue(Math.PI.toFixed(10)); setOverwrite(true); return; }

    const newDisplay = (overwrite || displayValue==='0' || displayValue.startsWith('Error')) ? (digit==='.'?'0.':digit) : displayValue+digit;
    setDisplayValue(newDisplay); setOverwrite(false);
  };
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 p-4 font-['Inter']">
      <div className='bg-[#1a1a1a] rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.8)] w-full max-w-lg p-6'>
        <div className='bg-[#2a2a2a] rounded-lg p-4 mb-4 shadow-inner min-h-[100px] flex flex-col justify-end'>
          <div className='text-xs text-gray-400 h-4 min-h-4 text-right overflow-hidden'>{prevValue} {operation && `[${operation}]`}</div>
          <div className='text-right text-4xl text-white font-light mt-1 break-words max-h-24 overflow-hidden'>
            <input className='text-right w-full bg-transparent outline-none pointer-events-none' type="text" value={message||displayValue} readOnly
              style={{ fontSize: message?'1.5rem':displayValue.length>15?'2rem':'3rem', color: message?'#f87171':'white' }} />
          </div>
        </div>
        ...
        <div className={`grid gap-3`} style={{ gridTemplateColumns: `repeat(${totalColumns}, minmax(0, 1fr))` }}>
          {allKeys.map((key) => {
            let action: () => void;
            if(key.value==='equals') action=equals;
            else if(key.value==='clear') action=clearDisplay;
            else if(key.value==='del') action=deleteLast;
            else if(!isNaN(Number(key.value)) || key.value==='.' || key.value==='e' || key.value==='pi') action=()=>setDigit(key.value);
            else action=()=>selectOperation(key.value);

            return (
              <Keyboard key={key.label} className={`text-white font-medium text-lg rounded-xl p-3 h-14 w-full ${key.style}`}
                onClick={action} style={{ gridColumn: key.span?`span ${key.span} / span ${key.span}`:'span 1 / span 1' }}>
                {key.label}
              </Keyboard>
            );
          })}
